...
// Lighting — required now that cubies use MeshStandardMaterial (was
// MeshBasicMaterial, which ignores light but also looked flat/plasticky).
scene.add(new THREE.AmbientLight(0xffffff, 0.7));
let learnKeyLight = new THREE.DirectionalLight(0xffffff, 0.8);
learnKeyLight.position.set(4, 6, 5);
scene.add(learnKeyLight);
let learnFillLight = new THREE.DirectionalLight(0xe8ab2e, 0.25);
learnFillLight.position.set(-3, -2, -4);
scene.add(learnFillLight);
...
function createCube(x, y, z) {
    ...
    // Crisp black edge lines so cubies read distinctly (real-cube plastic-gap look).
